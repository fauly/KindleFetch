...
"
        ...
        echo "9. Toggle insecure TLS verification: $INSECURE_TLS"
        echo ""
        ...
        case "$choice" in
            ...
            9)
                if [ "$INSECURE_TLS" = "true" ]; then
                    INSECURE_TLS=false
                    echo "Insecure TLS disabled"
                else
                    INSECURE_TLS=true
                    echo "Insecure TLS enabled (curl will skip certificate verification)"
                fi
            ...
            8)
                ...
                case "$choice" in
                    1)
                        ...
                        new_annas_url=$(normalize_url "$new_annas_url")
                    ...
                    3)
                        ...
                        read -r new_zlib_url
                        new_zlib_url=$(normalize_url "$new_zlib_url")

                        working_url=$(find_working_zlib_url "$new_zlib_url")
                        if [ -z "$working_url" ]; then
